...
async function findExercises() {
  ...
  try {
    ...
    api_server = 'https://mindscribe.rojan.hackclub.app'
...
if (saveTranscriptionBtn) {
  saveTranscriptionBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const response = await fetch('https://mindscribe.rojan.hackclub.app/transcribe', {
        method: 'POST',
        body: formData
      });
      if (!response.ok) {
  ...
  try {
    ...
    journalHistoryContainer.querySelectorAll('.journal-dropdown-item.view-full').forEach(item => {
      item.addEventListener('click', async function() {
        ...
        try {
          const resp = await fetch(`https://mindscribe.rojan.hackclub.app/analytics?analysis_id=${entryId}&user_email=${encodeURIComponent(userEmail)}`);
...
function showDeleteJournalConfirm(analysisId) {
  ...
  modal.querySelector('.delete-confirm-delete').onclick = async function() {
    ...
    try {
      ...
      const resp = await fetch('https://mindscribe.rojan.hackclub.app/delete-journal-history', {
        method: 'POST',
        body: formData
      });
